...
public final class PairingResult {
  ...
  private int playerMaxNameLength() {
    return result.stream().flatMap(game -> game.values().stream())
        .map(player -> player.name().length()).max(Integer::compareTo).orElseThrow();
  }

  private int powerMaxNameLength() {
    return result.stream().flatMap(game -> game.keySet().stream())
        .map(power -> power.name().length()).max(Integer::compareTo).orElseThrow();
  }
  ...
  public String toString() {
    ...
    var sortedPowers =
        result.getFirst().keySet().stream().sorted(Comparator.comparing(Power::toString)).toList();
    var sb = new StringBuilder();

    sb.append(" ".repeat(maxGameLength));
    sb.append(sortedPowers.stream().map(p -> String.format("%-" + cellWidth + "s", p.name()))
                  .collect(Collectors.joining(" | ")));
    sb.append("\n");

    sb.append(" ".repeat(maxGameLength));
    sb.append(
        sortedPowers.stream().map(p -> "-".repeat(cellWidth)).collect(Collectors.joining("-+-")));
    sb.append("\n");

    for (var i = 0; i < gameCount; i++) {
      var game = result.get(i);
      sb.append(gameLabelPrefix).append(String.format("%-" + maxGameIndexLength + "d", i))
